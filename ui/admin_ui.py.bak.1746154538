...
class AdminWindow(QMainWindow):
    ...
    def __init__(self, auth_controller: AuthController):
        try:
            ...
            raise
        # Criar aba de equipamentos por empresa
        self.create_company_equipment_tab()
    ...
    def initUI(self):
        ...
        try:
            ...
            self.equipment_table.setColumnCount(14)  # Atualizado para incluir os novos campos NR-13  # Atualizado para incluir os novos campos NR-13
            self.equipment_table.setHorizontalHeaderLabels([
                "Tag", "Categoria", "Empresa", "Fabricante", "Ano Fabricação", 
                "Pressão Projeto", "Pressão Trabalho", "Volume", "Fluido", "Status",
                "Categoria NR-13", "PMTA", "Placa Identificação", "Número Registro"  # Novos campos NR-13
            ])
            ...
            # --- Adicionar Filtros NR-13 ---
            filter_layout = QHBoxLayout()
            filter_layout.setSpacing(10)
            filter_layout.setContentsMargins(0, 10, 0, 10) # Espaçamento vertical

            # Filtro Tipo de Inspeção
            filter_layout.addWidget(QLabel("Tipo Insp.:"))
            self.insp_filter_tipo = QComboBox()
            self.insp_filter_tipo.addItems([
                "Todos", "Visual Externa", "Visual Interna", "Medição Espessura (ME)", 
                "Teste Hidrostático", "Calibração PSV", "Outro"
            ])
            self.insp_filter_tipo.setMinimumWidth(150)
            filter_layout.addWidget(self.insp_filter_tipo)

            # Filtro Categoria NR-13
            filter_layout.addWidget(QLabel("Cat. NR13:"))
            self.insp_filter_categoria = QComboBox()
            self.insp_filter_categoria.addItems(["Todas", "I", "II", "III", "IV", "V"])
            self.insp_filter_categoria.setMinimumWidth(80)
            filter_layout.addWidget(self.insp_filter_categoria)

            # Filtro Resultado
            filter_layout.addWidget(QLabel("Resultado:"))
            self.insp_filter_resultado = QComboBox()
            self.insp_filter_resultado.addItems(["Todos", "Aprovado", "Reprovado", "Aprovado c/ Restr."])
            self.insp_filter_resultado.setMinimumWidth(120)
            filter_layout.addWidget(self.insp_filter_resultado)

            # Filtro Data Realização
            filter_layout.addWidget(QLabel("Data Realiz.:"))
            self.insp_filter_data_inicio = QDateEdit()
            self.insp_filter_data_inicio.setCalendarPopup(True)
            self.insp_filter_data_inicio.setDisplayFormat("dd/MM/yyyy")
            self.insp_filter_data_inicio.setDate(QDate.currentDate().addMonths(-1)) # Padrão: último mês
            self.insp_filter_data_inicio.setFixedWidth(100)
            filter_layout.addWidget(self.insp_filter_data_inicio)
            
            filter_layout.addWidget(QLabel("até"))
            self.insp_filter_data_fim = QDateEdit()
            self.insp_filter_data_fim.setCalendarPopup(True)
            self.insp_filter_data_fim.setDisplayFormat("dd/MM/yyyy")
            self.insp_filter_data_fim.setDate(QDate.currentDate())
            self.insp_filter_data_fim.setFixedWidth(100)
            filter_layout.addWidget(self.insp_filter_data_fim)
            
            filter_layout.addStretch() # Empurra botões para direita

            # Botões de Filtro
            self.insp_filter_apply_btn = QPushButton("Aplicar Filtros")
            self.insp_filter_apply_btn.setIcon(self.create_icon_from_svg(self.icons['browse']))
            # self.insp_filter_apply_btn.clicked.connect(self.apply_inspection_filters) # Conectar depois de criar o método
            filter_layout.addWidget(self.insp_filter_apply_btn)

            self.insp_filter_clear_btn = QPushButton("Limpar")
            # self.insp_filter_clear_btn.clicked.connect(self.clear_inspection_filters) # Conectar depois de criar o método
            filter_layout.addWidget(self.insp_filter_clear_btn)

            inspection_layout.addLayout(filter_layout)
            # ---------------------------------
            ...
            # Adiciona a nova aba de "Equipamentos por Empresa"

            # Aba de Equipamentos por Empresa (reposicionada como segunda aba)
    ...
    def apply_theme(self):
        ...
        try:
            if self.is_dark:
                ...
                if hasattr(self, 'company_equipment_table') and self.company_equipment_table is not None:
            ...
            self.tabs.setTabIcon(1, self.get_tab_icon("equipamentos.png"))  # Ícone para a aba de Equipamentos por Empresa
        ...
        try:
            ...
            if modal.exec() == QDialog.Accepted:
                ...
                success, message = self.equipment_controller.criar_equipamento(
                    ...
                    # Campos NR-13
                    categoria_nr13=equipment_data['categoria_nr13'],
                    pmta=equipment_data['pmta'],
                    placa_identificacao=equipment_data['placa_identificacao'],
                    numero_registro=equipment_data['numero_registro']
                )
            ...
            if modal.exec_():
                ...
                tipo_acesso != user.get('tipo_acesso', '') or data['empresa'] != user.get('empresa', ''):
                    ...
                    if success:
                        ...
                        QMessageBox.critical(self, "Erro", f"Falha ao atualizar usuário: {message}")
                else:
    ...
    def refresh_all_tables(self):
        ...
        try:
            ...
            self.load_users()
            # self.load_equipment()  # Comentado, pois a aba de equipamentos foi removida

            # Atualiza equipamentos por empresa se houver empresa selecionada
            company_id = self.company_selector.currentData() if hasattr(self, 'company_selector') else None
            if company_id:
    ...
    def load_equipment_by_company(self, company_id=None):
        ...
        if hasattr(self, 'company_equipment_table') and self.company_equipment_table is not None:
            pass  # Bloco vazio para prevenir erro de indentação
        if not hasattr(self, 'company_equipment_table') or self.company_equipment_table is None:
            ...
            self.company_equipment_table.setColumnCount(13)  # Incluir os campos NR-13  # Incluir os campos NR-13
        ...
        try:
            ...
            for i, item in enumerate(company_equipment):
                ...
                categoria_nr13_item = QTableWidgetItem(item.get('categoria_nr13', ''))
                categoria_nr13_item.setFlags(categoria_nr13_item.flags() & ~Qt.ItemIsEditable)
                self.company_equipment_table.setItem(i, 9, categoria_nr13_item)
                
                pmta_item = QTableWidgetItem(str(item.get('pmta', '')))
                pmta_item.setFlags(pmta_item.flags() & ~Qt.ItemIsEditable)
                self.company_equipment_table.setItem(i, 10, pmta_item)
                
                placa_identificacao_item = QTableWidgetItem(item.get('placa_identificacao', ''))
                placa_identificacao_item.setFlags(placa_identificacao_item.flags() & ~Qt.ItemIsEditable)
                self.company_equipment_table.setItem(i, 11, placa_identificacao_item)
                
                numero_registro_item = QTableWidgetItem(item.get('numero_registro', ''))
                numero_registro_item.setFlags(numero_registro_item.flags() & ~Qt.ItemIsEditable)
                self.company_equipment_table.setItem(i, 12, numero_registro_item)
    ...
    def edit_company_equipment(self):
        ...
        try:
            ...
            success = self.edit_equipment(equipment_id)
    ...
    def create_company_equipment_tab(self):
        """Cria a aba de equipamentos por empresa."""
        try:
            company_equipment_tab = QWidget()
            company_equipment_tab.setObjectName("company_equipment_tab")
            self.equipment_by_company_tab_index = self.tabWidget.addTab(company_equipment_tab, "Equipamentos por Empresa")
            company_tab_layout = QVBoxLayout(company_equipment_tab)

            # Adicionar ComboBox para seleção de empresas
            company_selection_layout = QHBoxLayout()
            company_label = QLabel("Selecione a empresa:")
            self.company_combobox = QComboBox()
            company_selection_layout.addWidget(company_label)
            company_selection_layout.addWidget(self.company_combobox)
            company_selection_layout.addStretch(1)
            company_tab_layout.addLayout(company_selection_layout)

            # Carregar empresas para o ComboBox
            self.load_companies_to_combobox()

            # Criar tabela para equipamentos da empresa selecionada
            self.company_equipment_table = QTableWidget()
            self.company_equipment_table.setColumnCount(13)
            headers = ["ID", "Nome", "Tipo", "Número de Série", "Fabricante", "Ano de Fabricação", "Última Inspeção",
                       "Próxima Inspeção", "Categoria NR-13", "PMTA", "Placa Identificação", "Número Registro", "Ações"]
            self.company_equipment_table.setHorizontalHeaderLabels(headers)
            self.company_equipment_table.setEditTriggers(QAbstractItemView.NoEditTriggers)
            self.company_equipment_table.setSelectionBehavior(QAbstractItemView.SelectRows)
            company_tab_layout.addWidget(self.company_equipment_table)

            # Adicionar botões de ação para a tabela de equipamentos por empresa
            company_equipment_buttons_layout = QHBoxLayout()
            add_equipment_to_company_btn = QPushButton("Adicionar Equipamento")
            add_equipment_to_company_btn.clicked.connect(self.add_equipment_to_company)
            remove_equipment_from_company_btn = QPushButton("Remover Equipamento")
            remove_equipment_from_company_btn.clicked.connect(self.remove_equipment_from_company)
            company_equipment_buttons_layout.addWidget(add_equipment_to_company_btn)
            company_equipment_buttons_layout.addWidget(remove_equipment_from_company_btn)
            company_equipment_buttons_layout.addStretch(1)
            company_tab_layout.addLayout(company_equipment_buttons_layout)

            # Conectar sinal de mudança na seleção da empresa
            self.company_combobox.currentIndexChanged.connect(self.load_equipment_by_company)
        except Exception as e:
            print(f"Erro ao criar aba de equipamentos por empresa: {e}")
            logging.error(f"Erro ao criar aba de equipamentos por empresa: {e}")
